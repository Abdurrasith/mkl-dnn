...
public:
    ...
    virtual bool get_in_parallel() override { return 0; }
    virtual uint64_t get_flags() override { return 0; }
    virtual void parallel_for(
            int n, const std::function<void(int, int)> &fn) override {
        tbb::parallel_for(
...
public:
    ...
    virtual int get_num_threads() { return num_threads_; }

    virtual bool get_in_parallel() { return worker_self() != nullptr; }

    virtual uint64_t get_flags() { return 0; }
...
// by parallel*() callse from the tests.
namespace dnnl {
namespace impl {
